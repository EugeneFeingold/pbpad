...
class PreviewClient:
    ...
    def stop_nowait(self):
        ...
        The blocking socket close is offloaded to a shared executor (not our
        own, which is busy in getPreviewFrame). Closing the socket unblocks
        that call so the session-runner exits and the supervisor task ends."""
        self._stop = True
        pb = self._pb
        if pb is not None:
            loop = self._loop or asyncio.get_event_loop()
            loop.run_in_executor(None, self._safe_close, pb)
